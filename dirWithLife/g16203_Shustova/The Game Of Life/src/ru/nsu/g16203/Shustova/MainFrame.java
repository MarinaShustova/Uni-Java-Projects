...
public class MainFrame extends JFrame {
    ...
    protected ArrayList<JToggleButton> toggleButtons = new ArrayList<>();
    protected ArrayList<JButton> simpleButtons = new ArrayList<>();
...
//		getOpenFileName();
     ...
     * @param menuPath - path to menu item to create toolbar button from
     */
    public void addToolBarButton(String menuPath, boolean isToggle) {
        if (!isToggle)
            toolBar.add(createToolBarButton(menuPath));
        else
            toolBar.add(createToggleButton(menuPath));
    }

    public void linkToggleButtons() {
        for (int i = 2; i < 4; ++i) {
            int finalI = i;
            toggleButtons.get(i).addActionListener(e -> {
                for (int k = 2; k < 4; ++k)
                    if (!toggleButtons.get(k).equals(toggleButtons.get(finalI)))
                        toggleButtons.get(k).setSelected(false);
            });
        }
    }

    public void resetToggleButtons() {
        for (JToggleButton j : toggleButtons) {
            if (!j.equals(toggleButtons.get(1)))
                j.setSelected(false);
        }
    }
